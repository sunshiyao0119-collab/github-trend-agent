...
### 1.1 建立 GitHub API 最小采集闭环

目标：通过 GitHub Search API 获取少量公开仓库数据，并转换为项目内部的明确数据模型。

完成标准：

- 明确查询条件、请求参数和响应字段。
- 封装 HTTP 请求边界，不把网络逻辑写进 CLI。
- 获取并显示至少 3 个真实公开仓库。
- 对成功、GitHub API 错误和无效响应建立最小测试。
- 不记录或输出 GitHub Token。
